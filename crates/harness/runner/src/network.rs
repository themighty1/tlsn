...
impl Network {
    ...
    pub fn create(&mut self) -> Result<()> {
        ...
        self.ns_0.bring_up_loopback()?;
        self.ns_1.bring_up_loopback()?;
        self.ns_app.bring_up_loopback()?;
...
impl Namespace {
    ...
    /// Brings up the loopback interface in the namespace.
    fn bring_up_loopback(&self) -> Result<()> {
        duct::cmd!("sudo", "ip", "netns", "exec", &self.name, "ip", "link", "set", "lo", "up").run()?;
        Ok(())
    }
