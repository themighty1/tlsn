...
async fn main() {
    ...
    let (recv_private_ranges, recv_public_ranges) = find_ranges(
        ...
        &["\"screen_name\":\"chrischongtt\"".as_bytes()],
    );
    ...
    let mut file = tokio::fs::File::create("zkitter_twitter_name.json")
        .await
        .unwrap();
    file.write_all(
    ...
    let mut file = tokio::fs::File::create("zkitter_twitter_name_proof.json")
        .await
        .unwrap();
